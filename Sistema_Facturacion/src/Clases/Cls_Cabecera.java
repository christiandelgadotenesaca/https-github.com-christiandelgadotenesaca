...
public class Cls_Cabecera {
    ...
    public Cls_Cabecera(int FAC_CODIGO,String TRAN_CODIGO, String CLI_ID, String FAC_FECHA, String FAC_ESTADO, int FAC_PORCENTAJE, double FAC_TOTAL,String CLI_DIRECCION) {
    ...
    public void Guardar()
    {
     cnx.ejecutar("Insert Into factura_cabecera_tb (FAC_CODIGO,TRAN_CODIGO,CLI_ID,FAC_FECHA,FAC_ESTADO,FAC_PORCENTAJE,FAC_TOTAL,CLI_DIRECCION)values("+FAC_CODIGO+",'"+TRAN_CODIGO+"','"+CLI_ID+"','"+FAC_FECHA+"','"+FAC_ESTADO+"',"+FAC_PORCENTAJE+","+FAC_TOTAL+",'"+CLI_DIRECCION+"')");
    }
    public void CargarDatos(JTable tblFactura)
    {
    //select* sirve para selecionar todos los datos que existen en la tabla
     cnx.CargarTabla("SELECT FAC_CODIGO as FACTURA, CLI_ID AS CLIENTE, FAC_FECHA AS FECHA, FAC_ESTADO AS ESTADO,FAC_PORCENTAJE AS PORCENTAJE,FAC_TOTAL AS TOTAL FROM factura_cabecera_tb", tblFactura);
